...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_boba) {described_class.new('boba tea')}

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_boba.type).to be('boba tea')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_boba.ounces).to be(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_matcha) { described_class.new('matcha tea')}
      # remove the 'x' before running this test
      it 'is full' do
        expect(my_matcha).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_chai) { described_class.new('chai tea', 8)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_chai).not_to be_full
      end
    end
  end
end
